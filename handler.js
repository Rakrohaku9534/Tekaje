...
export async function handler(chatUpdate) {
	...
	try {
		...
		const isPrems = db.data.datas.prems.map(v => v.user).includes(m.sender)
...
export async function participantsUpdate({ id, participants, action }) {
    try {
      ...
      for (const jid of participants) {
         ...
         const text = (action === 'add' ? (chat.sWelcome || this.welcome || Connection.conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', this.groupMetadata.desc?.toString() || 'unknow') :
         (chat.sBye || this.bye || Connection.conn.bye || 'Bye, @user!')).replace('@user', '@' + jid.split('@')[0])         
         ...
         // action
         if (action == "add") { 
           if (!db.data.chats[id]?.welcome) return
            this.sendMessage(id, {
